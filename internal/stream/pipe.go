...
func (p *StreamPipe) Read(buf []byte) (n int, err error) {
	...
	if p.bytesRead >= p.totalBytes {
		ll.Debug("EOF (bytesread == contentLength)")
		return 0, io.EOF
	}
...
func (p *StreamPipe) prefetch() {
	...
	for currentBlock < totalBlocks {
		...
		if fetchErr != nil {
			if p.ctx.Err() == nil {
				ll.Error("block download failed", zap.Error(fetchErr))
			}
			return
		}
		...
		for _, block := range blocks {
			if block == nil {
				...
				ll.Error("unexpected nil block in batch, aborting prefetch")
				return
			}
			select {
...
func (p *StreamPipe) downloadBlock(ctx context.Context, offset int64) ([]byte, error) {
	ll := p.ll.Named("downloadBlock")
	ll.Sugar().Debugf("Downloading block at offset %d (block size: %d)", offset, p.blockSize)
...
func NewStreamPipe(
	...
	p := &StreamPipe{
		ctx:          ctx,
		cancel:       cancel,
		ll:           log.GetLogger(log.StreamModule),
